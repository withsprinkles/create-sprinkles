...
export default createTemplate({
    ...
    prepare({ take }) {
        return {
            author: async () => await take(inputFromScript, { command: "npm whoami" }),
        };
    },
    produce() {
        // ...
...
export default createTemplate({
    ...
    prepare({ runner }) {
        const npm = lazyValue(async () => await take(inputFromScript, { command: "npm whoami" }));
        const github = lazyValue(
            async () => await take(inputFromScript, { command: "gh api user -q .login" }),
        );
