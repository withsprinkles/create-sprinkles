...
Repositories created with Bingo default to local-only, without a remote upstream.
However, if the [CLI `--remote` flag](/cli#--remote) is enabled -either manually or from an `origin` Git remote existing- Bingo will send network requests to that remote repository.
